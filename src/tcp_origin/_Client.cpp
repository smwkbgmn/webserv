...
const message_t& Client::buffer() const { return in; }
...
message_t& Client::get_in() { return in; }
...
void Client::processClientRequest() {
	...
	log::print( "TCP\t: receiving data" );
    ssize_t byte = recv(client_socket, buf, SIZE_BUF, 0);
	log::print( "TCP\t: receiving done by " + std::to_string( byte ) );
    ...
    if (byte <= 0) {
		...
		try {
			if ( Transaction::recvMsg( in, buf, byte )) {
				log::history.fs << in.msg.str() << std::endl;
			...
			log::print( "HTTP\t: transaction: " + str_t( err.what() ) );
			...
			log::print( "HTTP\t: Request: " + str_t( err.what() ) );
...
bool Client::sendData() {
	...
	log::print( "TCP\t: sending\n" );
	log::history.fs << out.msg.str() << "\n" << std::endl;
